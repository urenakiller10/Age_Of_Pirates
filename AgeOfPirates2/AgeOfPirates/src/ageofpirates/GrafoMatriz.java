...
public class GrafoMatriz {
    ...
    public void agregarVertice(Factory v)
    {
    ...
    public boolean buscarFactory(int id){
        for (Vertice vertice : vertices) {
            if (vertice != null) {
                if (vertice.factory.getId() == id) {
                    return true;
                } 
            }
        }
    ...
    public Vertice buscarFactoryFac(int num){
        for (Vertice vertice : vertices) {
            if (vertice != null) {
                if (vertice.factory.getNum() == num) {
                    return vertice;
                }
            } 
        }
    ...
    public void cambiarMinas (int speed, int qty){
        for (int i = 0; i < vertices.length; i++) {
            if(vertices[i]!=null){
               if (vertices[i].factory.getName().equals( "Mine")){
                Mine mine= (Mine)vertices[i].factory;
                mine.setSpeed(speed);
